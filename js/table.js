...
function renderTable() {
    ...
    empData.slice(1).reverse().forEach((emp, revIdx) => {
        ...
        let msnv = "-"; let hoTen = safeStr(emp[0]); let gioitinh = safeStr(emp[4]); let ngayVao = safeDate(emp[5]);
        let ngayHD = "-"; let boPhan = safeStr(emp[6]); let chucVu = safeStr(emp[7]); let ngaySinh = safeDate(emp[3]);
        let cccd = safeStr(emp[19]); let ngayCap = safeDate(emp[20]); let quocTich = ""; 
        let thuongTru = safeStr(emp[18]).replace(/\|/g, ', '); let tamTru = safeStr(emp[38]); 
        let bhxh = safeStr(emp[33]); let mst = safeStr(emp[35]); let sdt = safeStr(emp[21]);
...
async function saveEmployeeData() {
    var idxStr = document.getElementById('emp_idx').value;
    ...
    emp[0] = document.getElementById('fm_hoten').value; emp[3] = document.getElementById('fm_ngaysinh').value;
    emp[4] = document.getElementById('fm_gioitinh').value; emp[5] = document.getElementById('fm_ngayvao').value;
    emp[6] = document.getElementById('fm_bophan').value; emp[7] = document.getElementById('fm_chucvu').value;
    emp[18] = document.getElementById('fm_thuongtru').value; emp[19] = document.getElementById('fm_cccd').value;
    emp[20] = document.getElementById('fm_ngaycap').value; emp[21] = document.getElementById('fm_sdt').value;
    emp[33] = document.getElementById('fm_bhxh').value; emp[35] = document.getElementById('fm_mst').value;
    emp[38] = document.getElementById('fm_tamtru').value;

    var btnSave = document.querySelector('.btn-save');
    var oldBtnHtml = btnSave.innerHTML;
    ...
    var payload = { action: isNew ? 'add' : 'edit', rowData: emp, sheetRow: isNew ? null : (idx + 1) };
    ...
    try {
        ...
        renderTable(); closeFormModal(); alert("✅ Đã lưu dữ liệu thành công!");
    } catch (e) {
        document.getElementById('formMsg').innerHTML = "❌ Lỗi: Không thể lưu. Hãy kiểm tra API Google Apps Script."; console.error(e);
